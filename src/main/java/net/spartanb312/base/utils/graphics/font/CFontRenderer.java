...
public class CFontRenderer extends CFont {
	...
	public float drawString(String text, double x, double y, int color, boolean shadow) {
		...
		if (render) {
			...
			GL11.glTranslatef((float) -x * 0.11111f, (float) -y * 0.11111f, 0.0f);
			GL11.glScalef(1.11111f, 1.11111f, 1.0f);
			...
			for (int i = 0; i < size; ++i) {
				...
				x += (currentData[character].width - 8 + this.charOffset) / 2.0f;
			}

			GlStateManager.enableBlend();
			GL11.glScalef(1.0f / 1.11111f, 1.0f / 1.11111f, 1.0f);
			GL11.glTranslatef((float) x * 0.11111f, (float) y * 0.11111f, 0.0f);
	...
	public String trimCFontStringToWidthBetter(String text, float width, boolean nearestWord) {
		...
		boolean nonSpaceEndingFlag = false;
		...
		for (int i = 0; i < size; ++i) {
			...
			if (nearestWord && String.valueOf(currentChar).equals(" "))
				lastSpaceIndex = i;

			if (getStringWidth(stringbuilder.toString()) >= width) {
				if (nearestWord && !String.valueOf(currentChar).equals(" "))
					nonSpaceEndingFlag = true;

				break;
			}
		}

		if (nearestWord && nonSpaceEndingFlag)
			stringbuilder.delete(lastSpaceIndex, stringbuilder.toString().length());
