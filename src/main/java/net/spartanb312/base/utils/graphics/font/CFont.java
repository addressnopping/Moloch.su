...
public class CFont {
	...
	public CFont(CustomFont font, boolean antiAlias, boolean fractionalMetrics) {
		try {
			Font inputFont = Font.createFont(Font.TRUETYPE_FONT, Objects.requireNonNull(CFont.class.getResourceAsStream(font.getFile()))).deriveFont(font.getSize() * 1.8f).deriveFont(font.getType());// creat CFont with ttf
			this.font = inputFont;
			this.antiAlias = antiAlias;
			this.fractionalMetrics = fractionalMetrics;
	...
	public void drawChar(CharData[] chars, char c, float x, float y) throws ArrayIndexOutOfBoundsException {
		try {
			drawQuad(x, y, chars[c].width / 2.0f, chars[c].height / 2.0f, chars[c].storedX, chars[c].storedY, chars[c].width, chars[c].height);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	protected void drawQuad(float x, float y, float width, float height, float srcX, float srcY, float srcWidth, float srcHeight) {
		...
		float renderSRCHeight = (srcHeight - 2.0f) / imgSize;
