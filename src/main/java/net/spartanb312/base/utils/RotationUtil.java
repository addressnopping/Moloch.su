...
public class RotationUtil extends ListenableContainer {
    ...
    public static void init() {
        BaseCenter.EVENT_BUS.register(new RotationUtil());
    }
    ...
    public static void resetRotation(boolean slowRotate, float degreesPerSecond) {
        if (shouldSpoofPacket) {
            if (slowRotate) {
                ...
                if (!(newYaw >= normalizedOriginalYaw - 2 && newYaw <= normalizedOriginalYaw + 2) && rotateTimer.passed(1)) {
    ...
    public static void lookAtTarget(Entity entity, boolean slowRotate, float degreesPerSecond) {
        ...
        if (slowRotate) {
            ...
            if (!(newYaw >= normalizedYaw - 1 && newYaw <= normalizedYaw + 1) && rotateTimer.passed(1)) {
